...
def get_user_profile(access_token: str) -> tuple[str, str]:
    """Returns (person_urn, display_name)"""
...
if not st.session_state.access_token:
    ...
    st.write("Click below to log in with your LinkedIn account. You'll be redirected back here after login.")
...
with col_logout:
    ...
    if st.button("Logout", use_container_width=True):
        for key in ["access_token", "person_urn", "user_name", "generated_caption"]:
            st.session_state[key] = None if key != "generated_caption" else ""
        st.rerun()
...
uploaded_images = st.file_uploader(
    ...
    help="Upload up to 9 JPG or PNG images"
)
        ...
        if st.button("🚀 Post Now", use_container_width=True):
            with st.spinner("Posting to LinkedIn..."):
                try:
                    ...
                    if uploaded_images:
                        st.info(f"Uploading {len(uploaded_images)} image(s)...")
                        image_urns = []
